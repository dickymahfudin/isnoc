...
class AjnLoggerController extends Controller
{
    ...
    public function sla($datas)
    {
        ...
        foreach ($datas as $value) {
            ...
            $up = ($sla > 100) ? 100 : $sla;
            $e1 = intval(array_sum($edl1) * 0.00027778);
            $e2 = intval(array_sum($edl2) * 0.00027778);
            $e3 = intval(array_sum($edl3) * 0.00027778);
            $v_min = round(min($batt_volt1) / 100, 1);
            $v_max = round(max($batt_volt1) / 100, 1);
            $v_avg = round((array_sum($batt_volt1) / count($batt_volt1)) / 100, 1);
            $dv = round($v_max - $v_min, 2);
            $e = $e1 + $e2 + $e3;
            $temph = round(((3 / 100) * $e) + $e, 2);
            $harvest1 = intval(((60 / 100) * $temph));
            $harvest2 = intval(((40 / 100) * $temph));
            $dataDown = (100 -  $up);
            array_push($result, [
                'date' => $date,
